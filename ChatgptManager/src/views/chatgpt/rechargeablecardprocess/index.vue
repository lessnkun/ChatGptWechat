  ...
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['chatgpt:rechargeablecardprocess:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
      ...
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['chatgpt:rechargeablecardprocess:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="使用者ID" prop="userId">
          <el-input v-model="form.userId" placeholder="请输入使用者ID" />
        </el-form-item>
        <el-form-item label="批次ID" prop="batchId">
...
<script>
import { listRechargeablecardprocess, getRechargeablecardprocess, delRechargeablecardprocess, addRechargeablecardprocess, updateRechargeablecardprocess } from "@/api/chatgpt/rechargeablecardprocess";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listRechargeablecardprocess(this.queryParams).then(response => {
    ...
    handleUpdate(row) {
      ...
      getRechargeablecardprocess(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateRechargeablecardprocess(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addRechargeablecardprocess(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除充值卡详情编号为"' + ids + '"的数据项？').then(function() {
        return delRechargeablecardprocess(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('chatgpt/rechargeablecardprocess/export', {
        ...this.queryParams
      }, `rechargeablecardprocess_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
