    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['chatgpt:rechargeablecardmain:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['chatgpt:rechargeablecardmain:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
      ...
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['chatgpt:rechargeablecardmain:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listRechargeablecardmain, getRechargeablecardmain, delRechargeablecardmain, addRechargeablecardmain, updateRechargeablecardmain } from "@/api/chatgpt/rechargeablecardmain";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listRechargeablecardmain(this.queryParams).then(response => {
    ...
    handleUpdate(row) {
      ...
      getRechargeablecardmain(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateRechargeablecardmain(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addRechargeablecardmain(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除充值卡-主编号为"' + ids + '"的数据项？').then(function() {
        return delRechargeablecardmain(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('chatgpt/rechargeablecardmain/export', {
        ...this.queryParams
