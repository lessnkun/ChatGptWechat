<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['chatgpt:roleChange:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
      ...
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['chatgpt:roleChange:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="roleChangeList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="角色设定语" align="center" prop="roleContent" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="角色图片" prop="roleImage">
          <image-upload v-model="form.roleImage"/>
        </el-form-item>
        <el-form-item label="角色设定语">
          <editor v-model="form.roleContent" :min-height="192"/>
        </el-form-item>
...
<script>
import { listRoleChange, getRoleChange, delRoleChange, addRoleChange, updateRoleChange } from "@/api/chatgpt/roleChange";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listRoleChange(this.queryParams).then(response => {
    ...
    handleUpdate(row) {
      ...
      getRoleChange(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateRoleChange(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addRoleChange(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('chatgpt/roleChange/export', {
        ...this.queryParams
