  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="操作类型(1:对话)" prop="operationType">
        <el-select v-model="queryParams.operationType" placeholder="请选择操作类型(1:对话)" clearable>
          <el-option
      ...
      <el-form-item label="对应的ID,如果对话则可以对应到具体内容" prop="operationId">
        <el-input
          ...
          placeholder="请输入对应的ID,如果对话则可以对应到具体内容"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['chatgpt:frequentcylog:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
      ...
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['chatgpt:frequentcylog:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="frequentcylogList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作类型(1:对话)" align="center" prop="operationType">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.operation_type" :value="scope.row.operationType"/>
        </template>
      </el-table-column>
      <el-table-column label="对应的ID,如果对话则可以对应到具体内容" align="center" prop="operationId" />
      <el-table-column label="变化内容" align="center" prop="changeContent" />
...
<script>
import { listFrequentcylog, getFrequentcylog, delFrequentcylog, addFrequentcylog, updateFrequentcylog } from "@/api/chatgpt/frequentcylog";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listFrequentcylog(this.queryParams).then(response => {
    ...
    handleUpdate(row) {
      ...
      getFrequentcylog(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateFrequentcylog(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addFrequentcylog(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('chatgpt/frequentcylog/export', {
        ...this.queryParams
