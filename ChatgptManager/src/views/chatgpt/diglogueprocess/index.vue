<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['chatgpt:diglogueprocess:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['chatgpt:diglogueprocess:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['chatgpt:diglogueprocess:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="diglogueprocessList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="询问内容" align="center" prop="askContent" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="询问内容">
          <editor v-model="form.askContent" :min-height="192"/>
        </el-form-item>
...
<script>
import { listDiglogueprocess, getDiglogueprocess, delDiglogueprocess, addDiglogueprocess, updateDiglogueprocess } from "@/api/chatgpt/diglogueprocess";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listDiglogueprocess(this.queryParams).then(response => {
    ...
    handleUpdate(row) {
      ...
      getDiglogueprocess(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateDiglogueprocess(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addDiglogueprocess(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('chatgpt/diglogueprocess/export', {
        ...this.queryParams
